...
function LoginButton({ togglePopup }) {
  // State để kiểm soát form đang hiển thị (Login / Reset Password / Sign Up)
  const [isResetPassword, setIsResetPassword] = useState(false);
  const [isSignUp, setIsSignUp] = useState(false);
  const [redirectToDashboard, setRedirectToDashboard] = useState(false); 
  if (redirectToDashboard) {
    return <Navigate to="/dashboard" />; // Redirect to dashboard if the state is true
  }
 

  // Hàm để kiểm tra nếu người dùng click ngoài popup
  const handleOutsideClick = (e) => {
    if (e.target.id === "overlay") {
      togglePopup(); // Đóng popup nếu người dùng click vào nền
    }
  };

  const handleForgotPassword = () => {
    setIsResetPassword(true); // Chuyển qua form Reset Password
  };

  const handleSignUp = () => {
    setIsSignUp(true); // Chuyển qua form Sign Up
  };

  const handleGoBackToLogin = () => {
    setIsResetPassword(false);
    setIsSignUp(false); // Quay lại form đăng nhập
  };
  ...
  return (
    <div
      ...
      onClick={handleOutsideClick} // Thêm sự kiện click vào vùng nền
    >
      <div
        className="bg-white p-8 rounded-lg w-1/3 relative"
        onClick={(e) => e.stopPropagation()} // Ngăn sự kiện click lan truyền khi click vào form
      >
        {/* Nút đóng "X" */}
        <button
            ...
            <form>
              ...
              <div className="relative mb-4">
                <input
                  ...
                  className="border border-[#2D3250] text-[#2D3250] focus:outline-none focus:ring-2 focus:ring-[#424769] hover:ring-2 hover:ring-[#424769] rounded-lg w-full p-2"
                />
              </div>
              <div className="w-full h-[2px] bg-[#7077A1] my-4"></div>
              <div className="flex justify-between mb-4">
                <a
                  href="#"
                  className="text-[#7077A1] hover:text-black text-sm"
                  onClick={handleForgotPassword} // Chuyển qua form Reset Password
                >
                ...
                <a
                  href="#"
                  className="text-[#7077A1] hover:text-black text-sm"
                  onClick={handleSignUp} // Chuyển qua form Sign Up
                >
                  Sign Up
                </a>
              </div>
